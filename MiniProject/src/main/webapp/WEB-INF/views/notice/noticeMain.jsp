	...
	<div class="content">
		...
		<div class="innerOuter" style="padding: 5% 10%;">
			...
			<c:if test="${loginUser.userId eq 'admin' }">
				<a class="btn btn-secondary" style="float: right; background-color: #2D55C9;" href="noticeInsertForm.ui">글쓰기</a>
				<br>
			</c:if>
			<br>
				...
				<ul class="pagination">
					<c:choose>
						...
						<c:otherwise>
							...
							<c:if test="${empty map }">
								<li class="page-item"><a class="page-link" href="noticeMain.ui?npage=${pi.currentPage-1}">&lt;이전</a></li>
							</c:if>
						</c:otherwise>
					</c:choose>
					<c:forEach var="i" begin="${pi.startPage }" end="${pi.endPage }">
						<c:choose>
							<c:when test="${i ne pi.currentPage }">
								...
								<c:if test="${empty map }">
									<li class="page-item"><a class="page-link" href="noticeMain.ui?npage=${i}">${i}</a></li>
								</c:if>
							</c:when>
							<c:otherwise>
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<c:if test="${empty map }">
								<li class="page-item"><a class="page-link" href="noticeMain.ui?npage=${pi.currentPage+1}">다음&gt;</a></li>
							</c:if>
						</c:otherwise>
					</c:choose>
				</ul>
			...
			<script>
				$(function() {
					$(".table>tbody>tr").click(function() {
						location.href = "noticeDetail.ui?vct=" + $(this).children().eq(0).text();
					})
				})
			</script>
			<br clear="both">
